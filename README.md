...
| `working_dir`            | Directory where Patcher should run, relative to the repository root. If empty, it runs in the whole repo. The action converts this to an absolute path when calling Patcher so path resolution is consistent between report and update. |                                                |
...
| `spec_file`              | Filename for the upgrade specification, relative to the repository root. The action converts this to an absolute path when calling Patcher so path resolution is consistent between report and update.   | `spec.json`                                    |
